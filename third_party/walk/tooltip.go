...
func (tt *ToolTip) addTool(hwnd win.HWND, track bool) error {
	...
	if win.FALSE == tt.SendMessage(win.TTM_ADDTOOL, 0, uintptr(unsafe.Pointer(&ti))) {
		// Some Windows/theme combinations reject tooltip registration for
		// otherwise valid controls. Tooltips are non-essential, so do not fail
		// the whole window creation when the OS refuses this helper control.
		return nil
	}
